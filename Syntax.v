...
(*
  ...
  return_ (add (hd,a) (shift Γ')).
*)
Admitted.


Definition extend_fmap (Q : QType) {X Y : Type} (f : X -> Y) (x : Extend Q X) : Extend Q Y :=
  match x with
  | hd => hd
  | tl x => tl (f x)
  end.

Instance extend_functor Q : Monad.Functor (Extend Q) := {fmap := @extend_fmap Q}.
...
Definition shift {X} {Q R : QType} (e : Exp X R) : Exp (Extend Q X) R := exp_fmap tl e.
